...
(function () {
    ...
    amGloble.page.fuyu_repairList = new $.am.Page({
        id: "fuyu_repairList",
        ...
        beforeShow: function (paras) {
            ...
            if(paras && paras.carRepairs)
            {
                var contentWrapper = self.$.find(".content_wrapper").empty();
                $.each(paras.carRepairs, function () {
                    var currentInfo = this;
                    var content = '<div class="menu-item am-clickable"><div class="text"><div class="title">' + this.vsstName + '</div><div class="date">'
                        + this.vrepairDate + '</div></div><div class="price">' + this.nbalanceFee + '<sup class="small">￥</sup></div><div class="button"></div></div>';
                    var contentObj = $(content).vclick(function () {
                        $.am.changePage(amGloble.page.fuyu_repairDetail, "slideleft", { vin: paras.vin, repairId: currentInfo.repairId });
                    });

                    contentWrapper.append(contentObj);
                });
            }
