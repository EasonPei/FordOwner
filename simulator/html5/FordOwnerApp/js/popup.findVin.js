...
(function() {
	...
	amGloble.popup.findVin = new $.am.Popup({
		id : "popup_findVin",
		init : function() {
		...
		beforeShow : function(option) {
			...
			this.$.find(".popViewCloseBtn").vclick(function() {
				self.$.fadeOut(500,function(){
					self.hide();
				})
			});
			if (amGloble.setting.market.code == "CN"){
				this.$.find("#findVinImage").hide();
			}
			else {
				this.$.find("#findVinImageCN").hide();
			}
		},
		afterShow : function() {
		},
