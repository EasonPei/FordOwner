...
(function () {
    ...
    amGloble.page.knowledge_phasebook = new $.am.Page({
        id: "knowledge_phasebook",
        // phraseBook_self : null,
        // phraseBook : null,
        // downloaStorageInfo : {},
...
//                });
        ...
        beforeShow: function (paras) {
            ...
            if (phraseBook_self.isNeedToDownload()) {
                phraseBook_self.clearResources();
                phraseBook_self.updateVersionInfo();
            }
        ...
        generatephraseBookPage: function (ret) {
            ...
            //console.log(ret);
            var root = phraseBook_self.$.find(".content_wrapper");
            root.empty();
            var imgs = [];
...
//                        });
                        ...
                        if (name instanceof Array) {
                            $.each(name, function () {
                                ...
                                if (soundDownloadStorageInfo[storeName] != null) {
                                    phraseBook_self.checkSoundExist(soundDownloadStorageInfo[storeName].fullPath, soundDownloadStorageInfo[storeName].nativeURL, $this);
                                } else {
                                    phraseBook_self.downloadSound($this);
                                }
                            });
                        }
                        else {
                ...
                if (amGloble.setting.market.code == "AU") {
                    if (phraseBook[i].category == "General") {
                        submenu.append("<div class='disclaimer'>" + i18n.PHRASE_GENERAL_DISCLAIMER + "</div>");
                    } else if (phraseBook[i].category == "Text Messages") {
                ...
                title.unbind("vclick").vclick(function () {
                    ...
                    //phraseBook_self.scrollview && phraseBook_self.scrollview.scrollTo('top');
                    phraseBook_self.scrollview && phraseBook_self.scrollview.refresh();
                });
            ...
            if (pbIconDownloadStatus == null) {
                ...
                for (var i = 0; i < imgs.length; i++) {
                    phraseBook_self.downloadPBIcon(imgs[i]);
                }
            } else {
                ...
                for (var i = 0; i < imgs.length; i++) {
                    ...
                    if (downloaStorageInfo[storeName] != null) {
                        ...
                        phraseBook_self.checkImageExist(filepath, nativeUrl, image);
                    } else {
                        phraseBook_self.downloadPBIcon(image);
                    }
                }
            }
            phraseBook_self.scrollview && phraseBook_self.scrollview.scrollTo('top');
        ...
        downloadPBIcon: function (img) {
            ...
            emap.directDownload(opt, function (ret) {
                if (ret.fullPath != null) {
                    ...
                    phraseBook_self.downloadTaskFinish();
                ...
                $.each(name, function () {
                    ...
                    emap.directDownload(opt, function (ret) {
                        if (ret.fullPath != null) {
                            ...
                            // phraseBook
                            // img.css("background-image", "url(" + ret.nativeURL + ")");
                            phraseBook_self.play(ret.nativeURL.replace("file://", ""), btn);
        ...
        checkImageExist: function (filepath, nativeUrl, img) {
            ...
            cordova.exec(function (ret) {
                if (ret) {
                    ...
                    phraseBook_self.downloadPBIcon(img);
                }
            }, function () {
            }, "File", "testFileExists", opt);
        ...
        checkSoundExist: function (filepath, nativeUrl, btn) {
            ...
            cordova.exec(function (ret) {
                if (ret) {
                    phraseBook_self.play(nativeUrl.replace("file://", ""), btn);
                } else {
                    phraseBook_self.downloadSound(btn);
                }
            }, function () {
            }, "File", "testFileExists", opt);
